...
from app.utils.clients import find_user_by_email
...
def share_itinerary(current_user_id, itinerary_id):
    """Share an itinerary with another registered user by email. Resolves
    the email via user-service -- this service owns no user data itself."""
    data = request.get_json(silent=True) or {}
