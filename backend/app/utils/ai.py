...
def answer_destination_question(destination: dict, question: str) -> str:
    ...
    """
    # No credentials resolvable at all makes the SDK raise a plain TypeError
    # at request-build time, not an anthropic.AnthropicError -- check first
    # instead of trying to catch that.
    ...
    try:
        ...
        # No API key configured, network failure, rate limit, etc. -- degrade
        # to the rule-based answer instead of erroring out.
        return _fallback_answer(destination, question)
